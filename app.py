...
def readreview(itemid):
    try:
        cursor = mydb.cursor(buffered=True)
        # Fetch item details
        cursor.execute("""SELECT bin_to_uuid(itemid),item_name,item_description,item_about,item_price,item_quantity,item_category,item_filename FROM WHERE itemid = uuid_to_bin(%s)""", [itemid])
        item_data = cursor.fetchone()
        # Fetch reviews
        cursor.execute("""SELECT ir.review_text, ir.rating,u.username FROM  ir JOIN userdata u ON ir.userid = u.userid WHERE ir.itemid = uuid_to_bin(%s) ORDER BY ir.reviewid DESC""", [itemid])
